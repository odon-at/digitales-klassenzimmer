...
## ✅ Umsetzung v0.13.0 (Klick-Fehler behoben, ein Ablageziel, Stadt-Texte)

### Der gemeldete Fehler – vier Ursachen, die sich überlagert haben

„Es funktioniert noch immer nicht, dass man mit der Maus einen leuchtenden Punkt
auswählt … deshalb können die Personen nie die Aufgabenstellung sehen." Der Fehler war
reproduzierbar, und er hatte **nicht eine, sondern vier** Ursachen:

| # | Ursache | Wirkung | Behebung |
|---|---|---|---|
| 1 | `setPointerCapture` lief schon bei `pointerdown` auf dem Viewport | Der Browser leitet den folgenden `click` auf den Viewport um – der Klick-Handler des Hotspots **lief nie**, das Panel öffnete nicht | Der Zeiger wird erst eingefangen, wenn er **6 px** zurückgelegt hat. Ein Tippen bleibt ein Klick. |
| 2 | Der `focus`-Handler drehte die Kamera | Browser fokussieren Knöpfe bereits bei `mousedown` – die Kamera fuhr los, **bevor** der Klick überhaupt ausgewertet war | Gedreht wird nur noch bei **Tastatur**-Fokus (`:focus-visible`, in `try/catch`). |
| 3 | Keine Bewegungsschwelle, dazu `dt = Math.max(1, …)` | Bei schnellen Mäusen ergab ein 20-px-Zucken rund `32°/Frame` – mit dem Ausrollen **etwa 400 Grad Drehung**. Genau die gemeldete „360-Grad-Drehung". | Echtes `dt` (mind. 8 ms) und ein Höchstwert von `6°/Frame`. Der Nachlauf liegt jetzt gemessen bei rund **75°** statt 400°. |
| 4 | `.city-spot[aria-hidden="true"] { pointer-events: none }` | Das Panorama zeigt jede Kachel doppelt. Ab Blickrichtung 262° ist die **Zweitkopie** der sichtbare Hotspot – dort ging jeder Klick garantiert ins Leere. | `pointer-events: auto`. Der Klick-Handler hing ohnehin an beiden Kopien; `aria-hidden` und `tabindex="-1"` bleiben, damit Screenreader nicht doppelt vorlesen. |

Zusätzlich: nur die **linke** Maustaste dreht (`button !== 0` wird ignoriert), und der
Viewport hat `touch-action: none`, damit Tablets die Geste nicht abfangen.

> **Bewusste Abweichung (unverändert seit v0.12.0):** **Tabulator und Enter bleiben.**
> Die Spec fordert das in diesem Abschnitt ausdrücklich selbst („für Leute, die keine
> Maus besitzen"). Beides ist normale Browserbedienung, kein Tastenkürzel.

**Regressionsschutz:** Die Testsuite `tcity` fährt jetzt die echte Ereigniskette
(`pointerdown → pointerup → click`) und prüft: Das Panel geht auf, die Kamera bleibt
stehen, 4 px gelten noch als Klick, ab 6 px wird gedreht **ohne** zu öffnen, der
Nachlauf bleibt unter einer halben Umdrehung, und auch die Zweitkopie ist klickbar.

### Ein Ablageziel statt doppelter Diagrammnamen

Die zweite Reihe (`.l4-slots`) mit denselben drei Diagrammnamen ist **ersatzlos
entfallen**. Nach der richtigen Typwahl wird der **oben gewählte Diagramm-Knopf selbst
zum Ablageziel** (gestrichelter, leuchtender Rahmen); die beiden anderen werden blass.
Darunter steht die geforderte **Sprechblase**:

> 💬 Ziehe die Daten „Feinstaub-Messreihe" zum ausgewählten Diagramm, um die Daten in
> die Diagrammart zu konvertieren.

Das fertige Diagramm baut sich anschließend in einem eigenen Kasten an derselben Stelle
auf. Ziehen **und** Anklicken funktionieren unverändert, ebenso „Taste 1/2/3".
**Punkte und Zählung sind unangetastet:** 16 Einheiten, 2500 Punkte bei fehlerfreiem
Durchlauf.

*Nebenbei bereinigt:* Das Instruktions-Banner sagte in Schritt 2 fast wörtlich dasselbe
wie die neue Sprechblase. Es meldet jetzt nur noch den Zustand
(„✓ Richtige Diagrammart. Jetzt die Daten übertragen."), das *Wie* erklärt die Blase.

### Cyber-Stadt: Texte und Gebäudenamen

| vorher | jetzt |
|---|---|
| „0 / 10 geprüft" | **„0 / 10 besucht"** |
| langer Intro-Text mit Dreh-Anleitung über dem Knopf | kurz, vor dem Betreten: „Das Bürger-Portal läuft. Betritt jetzt die Stadt und prüfe, welche Daten offen bleiben dürfen – und welche streng privat sind." |
| — | dauerhaft **in** der Stadt, oben mittig: „🖱 Klicke auf ein Gebäude in der Stadt, um die Aufgabe zu öffnen." |

Alle zehn Marker tragen jetzt Ortsnamen statt Datenkategorien:
**Fitnessstudio · Bank · Messstation · Parkbank · Bushaltestelle · Wohnhaus ·
Parkplatz · Krankenhaus · Stadtplan-Terminal · Wohnungstür.**
Die ausführlichen Titel im Entscheidungs-Panel bleiben unverändert – sie erklären ja
gerade, *welche Daten* an diesem Ort sichtbar sind.

> **Kleine Abweichung im Wortlaut:** Die Spec schlägt „Klicke auf ein Gebäude in der
> Stadt, um in die Stadt einzutreten" vor. Der Satz steht aber **in** der Stadt – man
> ist also schon drin. Umgesetzt ist deshalb „… um die Aufgabe zu öffnen", weil genau
> das der Klick bewirkt und der Satz sonst in sich widersprüchlich wäre.

Die Screenreader-Ansage beim Betreten beschreibt den neuen Bedienweg: Gebäude
anklicken, mit der Maus ziehen oder die ‹ ›-Knöpfe nutzen, ohne Maus mit Tabulator
von Gebäude zu Gebäude und Enter zum Öffnen.
