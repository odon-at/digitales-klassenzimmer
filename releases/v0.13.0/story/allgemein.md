...
**Browser-Stimme für VORLESEN.** Der Knopf nutzt jetzt ausnahmslos die native
Sprachausgabe – für alle Avatare, in Tipps, Hilfen und Info-Fenstern. Die
MP3-Vorrangschaltung aus v0.10.0 ist samt Zuordnungstabelle und `clipKey` **entfernt**.
Praktisch änderte das nichts am Erlebnis (die Tabelle war leer, es sprach ohnehin immer
die Browser-Stimme) – entfernt wurde toter Code und eine irreführende Beschreibung.
Details: [stimme.md](stimme.md). Die **einzige** verbliebene feste Aufnahme vertont die
Einleitung ([einleitung.md](einleitung.md)).

**Oranger Info-Knopf.** `ℹ INFO · 0 PKT` trägt in allen Leveln die Klasse `.btn-info`:
kräftiger oranger Rahmen, warm-oranger Grund, oranger Text. Der Puls wechselt stufenlos
zwischen Dunkelorange (`--orange #ff8c1a`) und hellerem Orangerot (`--orange-hot
#ff5e1a`), und der Lichtschein dehnt sich dabei aus und zieht sich zusammen. Der
gleiche Knopf steht auch in Level 3 im Level-Körper und in Level 1 in Schritt 3.

> **Technischer Hinweis:** Der Schein läuft über `filter: drop-shadow()` und **nicht**
> über `box-shadow`. `.btn` hat ein `clip-path` – ein `box-shadow` würde außerhalb des
> Polygons abgeschnitten und wäre kaum sichtbar. Bei `prefers-reduced-motion` bleibt
> ein statischer oranger Schein ohne Puls.

**Hintergrundmusik.** Geliefert wurde eine 13,3 s lange Stereo-**WAV** mit
`.mp3`-Endung (2,2 MB). Ausgeliefert wird die daraus erzeugte AAC-Fassung
`game/media/musik-loop.m4a` (**214 KB**); die Originaldatei bleibt im Repo. Alles
Weitere in [levels/musik.md](levels/musik.md).
