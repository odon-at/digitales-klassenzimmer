...
(function (NX) {
  ...
  /* ---- Sprachleiste: EIN Vorlese-Knopf (story/stimme.md) -----------------
     ...
     Gesprochen wird IMMER von der nativen Browser-Stimme (Web Speech API) –
     für alle Avatare, ohne Ausnahme (story/allgemein.md, „Nutzung der
     Browser-Stimme für die Vorlesefunktion"). Feste MP3-Aufnahmen kommen an
     dieser Stelle nicht mehr zum Einsatz.
  ...
  function voiceBar(getText, opts) {
    ...
    readBtn.addEventListener('click', function () {
      if (!A) return;
      if (A.voice && A.voice.isActive()) { stop(); return; }
      speakText();
    });
  ...
  function showModal(title, bodyHtml, opts) {
    ...
    if (opts && opts.speak) {
      ...
      body.insertBefore(voiceBar(function () { return speakText; }), body.firstChild);
    }
    m.hidden = false;
  }
  function closeModal() {
