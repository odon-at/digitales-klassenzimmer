...
(function (NX) {
  ...
  NX.levelDefs.push({
    ...
    quality: 0,
    // 16 bewertete Teilaufgaben: 3 Diagrammwahlen + 3 Zuordnungen + 10 Entscheidungen
    scoreUnits: 16,
    tips: [
    ...
    mount: function (container, ctx) {
      ...
      var builtCount = 0;
      var wrongType = 0;      // Fehlversuche bei der Diagrammwahl (gesamt)
      var typeRetries = 0;    // Aufgaben, die NICHT im Erstversuch saßen
      var wrongHereType = 0;  // Fehlversuche bei der AKTUELLEN Aufgabe

      /* Nur EIN keydown-Handler gleichzeitig: Phase A hört auf 1/2/3, die
         360°-Ansicht auf die Pfeiltasten. Beim Wechsel wird getauscht, damit
         kein Kürzel aus der Vorphase in der Stadt weiterwirkt. */
      function setKeyHandler(fn) {
        if (lb.keyDown) document.removeEventListener('keydown', lb.keyDown);
        lb.keyDown = fn || null;
      ...
      var decisionRetries = 0; // falsch entschiedene Szenarien (kein Erstversuch)
      ...
      /* Teilaufgaben: 3 Diagrammwahlen + 3 Chip-Zuordnungen + 10 Entscheidungen.
         Die Zuordnung selbst ist nicht bewertbar (nur ein Feld ist freigeschaltet)
         und zählt deshalb immer als Erstversuch. */
      function units() { return correctTypeCount + builtCount + decided; }
      function firstTries() {
        return (correctTypeCount - typeRetries) + builtCount + (decided - decisionRetries);
      }
      function scoreNow() {
        return NX.score.levelScore({ units: units(), firstTry: Math.max(0, firstTries()), wrong: wrongType + wrongDecision });
      }
      ...
      function askChart(i) {
        ...
        banner = el('div', { class: 'l4-banner', role: 'status' }, [
          el('span', { class: 'l4-banner-ico', text: '👉' }),
          el('span', { class: 'l4-banner-txt', text: d.banner.step1 })
        ]);
        task.appendChild(banner);
        ...
        d.typen.forEach(function (t) {
          ...
          b.dataset.type = t.id;   // dient nach der Wahl zugleich als Ablageziel
        ...
        task.appendChild(el('div', { class: 'l4-chipbar' }, [chip]));

        /* Keine zweite Reihe mit denselben Diagrammnamen mehr (Spec
           „Verbesserung der Daten Metropole"): Abgelegt wird direkt auf dem
           oben gewählten Diagramm. Die Sprechblase sagt genau das. */
        var bubble = el('div', { class: 'l4-bubble', role: 'status', hidden: 'hidden' }, [
          el('span', { class: 'l4-bubble-ico', 'aria-hidden': 'true', text: '💬' }),
          el('span', { class: 'l4-bubble-txt', text: d.bubble.replace('{chip}', c.chip) })
        ]);
        task.appendChild(bubble);

        // Ablageziele sind die Diagramm-Knöpfe selbst
        d.typen.forEach(function (t, i) { slotEls.push(typeBtns[i]); });

        body.appendChild(task);
        try { typeBtns[0].focus({ preventScroll: true }); } catch (e) { /* ältere Browser */ }

        /* „Taste 1/2/3" steht sichtbar an den Knöpfen – also muss sie auch
           wirken (gleiches Muster wie die Verdikte in Level 3). */
        setKeyHandler(function (e) {
          if (armedSlot) return;                     // Wahl steht schon
          var modal = document.getElementById('modal');
          if (modal && !modal.hidden) return;
          var tag = e.target && e.target.tagName;
          if (tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'SELECT') return;
          var i = ['1', '2', '3'].indexOf(e.key);
          if (i < 0 || !typeBtns[i] || typeBtns[i].disabled) return;
          e.preventDefault();
          typeBtns[i].click();
        });
        ...
        function setBanner(text, ico) {
          if (!banner) return;
          banner.querySelector('.l4-banner-txt').textContent = text;
          if (ico) banner.querySelector('.l4-banner-ico').textContent = ico;
        }
        ...
        function armSlot(typeId) {
          slotEls.forEach(function (s) {
            ...
            var lbl = d.typen.filter(function (t) { return t.id === s.dataset.type; })[0].label;
            s.setAttribute('aria-label', match ? lbl + ' – Zielfeld für die Daten' : lbl);
        ...
        function buildChart(cfg, slot) {
          ...
          if (chartIdx < 2) {
            ...
            task.appendChild(el('p', { class: 'hint', style: 'text-align:center;margin:14px 0 0',
      ...
      function enterCity() {
        ...
        announce('360°-Stadt. Klicke ein leuchtendes Gebäude an, um die Aufgabe zu öffnen. ' +
          'Zum Umsehen mit der Maus ziehen oder die Pfeil-Knöpfe am Rand nutzen; ' +
          'ohne Maus springt der Tabulator von Gebäude zu Gebäude, Enter öffnet. ' +
          '10 Gebäude zu besuchen.');
        ...
        var pointerActive = false, pStartX = 0, pStartY = 0, pStartH = 0, pLastX = 0, pLastT = 0;
        var nearestIdx = -1;
        ...
        var progress = el('div', { class: 'city-progress', text: '0 / ' + TOTAL + ' besucht' });
        ...
        /* Steht dauerhaft über der Stadt: Ohne diesen Satz haben Spielende laut
           Rückmeldung nicht erkannt, dass die Leuchtpunkte anklickbar sind. */
        var cityHint = el('div', { class: 'city-hint', text: d.city.intro.hint });

        var panel = buildPanel();

        var viewport = el('div', { class: 'city-viewport' }, [layerSky, layerFar, layerCity, layerFore]);
        var stage = el('div', { class: 'city-stage' }, [viewport, glitchEl, hero, navPrev, navNext, progress, compass, cityHint, panel]);
        ...
        function buildSpot(sp, i, isPrimary) {
          ...
          if (isPrimary) {
            /* Nur beim Tastatur-Fokus mitdrehen. Browser fokussieren Knöpfe
               schon bei mousedown – vorher fuhr die Kamera deshalb bei jedem
               Mausklick los, noch bevor der Klick ausgewertet war. */
            btn.addEventListener('focus', function () {
              lastFocused = btn;
              var byKeyboard = false;
              try { byKeyboard = btn.matches(':focus-visible'); } catch (e) { byKeyboard = false; }
              if (byKeyboard && !spinLocked) turnTo(sp.angle);
            });
          }
        ...
        /* Jeder Hotspot existiert zweimal (nahtloser Streifen) – Statuswechsel
           müssen immer beide Kopien treffen. */
        function spotNodes(i) {
          return ui.$$('.city-spot[data-i="' + i + '"]', stage);
        }
        ...
        /* ---- Eingabe: Pointer-Drag mit Schwung ----------------------------
           Wichtig und in v0.13.0 repariert: Der Zeiger wird ERST eingefangen,
           wenn wirklich gezogen wird (6-px-Schwelle wie beim Chip in Phase A).
           Vorher lag das setPointerCapture im pointerdown – dadurch wurde der
           anschließende click auf den Viewport umgeleitet und erreichte den
           Hotspot-Knopf nie: Die Frage öffnete sich nicht, stattdessen drehte
           sich die Stadt. Genau der gemeldete Fehler.                       */
        var pDown = false, pMoved = false;
        var VEL_MAX = 6;   // Grad je Frame -> höchstens rund 75 Grad Auslauf

        viewport.addEventListener('pointerdown', function (e) {
          if (spinLocked) return;
          if (!e.isPrimary) return;
          if (e.pointerType === 'mouse' && e.button !== 0) return;  // nur linke Taste
          pDown = true; pMoved = false;
          pStartX = e.clientX; pStartY = e.clientY; pStartH = h;
        ...
        viewport.addEventListener('pointermove', function (e) {
          ...
          if (!pMoved) {
            ...
            if (dx * dx + dy * dy < 36) return;   // 6 px – ein Tippen bleibt ein Klick
            pMoved = true; pointerActive = true; turnTarget = null; vel = 0;
          ...
          /* Echte Zeitdifferenz statt Dauer-1: sonst wurde vel bei schnellen
             Mäusen um ein Vielfaches zu groß und ein Zucken drehte die Stadt
             einmal ganz herum. */
          var now = Date.now();
          var dt = Math.max(8, now - pLastT);
          var v = -((e.clientX - pLastX) / PX_PER_DEG) * (16 / dt);
        ...
        /* Reines Maus-Verhalten (Spec „Verbesserung bei der Cyber-City"):
           gedreht wird per Maus-Ziehen und über die ‹ ›-Knöpfe, es gibt keine
           Tastenkürzel mehr. Der Phase-A-Handler wird hier abgeräumt.
           Tabulator und Enter bleiben – das ist keine Tastenkombination,
           sondern die normale Browserbedienung und der einzige Zugang ohne
           Maus; ohne sie wäre das letzte Level nicht abschließbar. */
        setKeyHandler(null);
        ...
        /* ---- Entscheidungs-Panel ---- */
        var CITY_Q = 'Diese Daten sind sichtbar. Was tust du?';
        function buildPanel() {
        ...
        function showPanel(i, alreadyDone) {
          ...
          if (alreadyDone) {
            ...
            panel.appendChild(el('p', { class: 'city-panel-q' }, [
              el('span', { class: 'city-panel-q-ico', 'aria-hidden': 'true', text: '❓' }),
              el('span', { text: CITY_Q })
            ]));
            var btns = [];
            var choices = el('div', { class: 'city-choices' }, d.city.answers.map(function (ans) {
              var b = el('button', { class: 'city-btn', type: 'button' }, [
                el('span', { class: 'city-btn-ico', 'aria-hidden': 'true', text: ans.icon }),
          ...
          announce(sp.title + '. ' + sp.desc + (alreadyDone ? '' : ' ' + CITY_Q));
        }
        ...
        function resolveDecision(i, answerId, btn, allBtns) {
          ...
          var badge = ok
            ...
            : 'is-wrong';
          // beide Kopien markieren, sonst wirkt die zweite unbearbeitet
          spotNodes(i).forEach(function (n) {
            n.classList.add('is-done', badge);
            n.setAttribute('title', 'Bereits entschieden');
          });
          ...
          if (sp.correct === 'open') {
            openDecided++;
            // Leaderboard-Spalte „Gefundene Open Data Sets“
            if (ok) NX.state.update({ openDataSets: (NX.state.get().openDataSets || 0) + 1 });
          }
          if (openDecided === 5) ctx.markTask(3);
          updateQuality();
          updateHud('360°-Stadt · ' + decided + ' / ' + TOTAL + ' entschieden');
          progress.textContent = decided + ' / ' + TOTAL + ' besucht';
        ...
        function cityFinale() {
          ...
          later(function () {
            /* Award-Panel abräumen, BEVOR der Host sein Erfolgs-Overlay setzt.
               Sonst stehen zwei deckende Overlays übereinander – genau das ließ
               den „ZUM FINALE"-Knopf verschwinden (Spec belohnung.md). */
            if (fin.parentNode) fin.parentNode.removeChild(fin);
            ctx.complete({
              units: units(), firstTry: Math.max(0, firstTries()),
              wrong: wrongType + wrongDecision
            });
