...
(function (NX) {
  ...
  NX.levelDefs.push({
    ...
    quality: null,
    // 6 bewertete Teilaufgaben: die sechs Meldungen
    scoreUnits: 6,
    ...
    mount: function (container, ctx) {
      ...
      var firstTryCount = 0;// Fragen, die ohne Fehlversuch saßen (Erstversuch-Bonus)
      var wrongHere = 0;    // Fehlversuche bei der AKTUELLEN Frage
      ...
      var infoBtn = el('button', { class: 'btn btn-ghost btn-sm btn-info', text: 'ℹ️ Info (0 Punkte)', onclick: openInfo });
      ...
      function onVerdict(vId, btn) {
        ...
        if (vId === q.correct) {
          answered = true;
          if (wrongHere === 0 && !tipsPaid[idx]) firstTryCount++;
      ...
      function openTip() {
        ...
        ui.showModal('💡 TIPP · −' + sc.tip + ' Punkte',
          '<p>' + ui.escapeHtml(q.help.tip) + '</p>', { speak: q.help.tip });
      }

      /* ---------------- HUD ---------------- */
      /* Live-Anzeige auf der neuen Skala (score.js). Die endgültige Wertung
         inkl. Zeit-Bonus rechnet der Level-Host in completeLevel(). */
      function scoreNow() {
        /* Tipps als Fehlversuch-Äquivalent MIT in levelScore geben – sonst
           landet die Anzeige nach einem frühen Tipp im Minus. */
        return NX.score.levelScore({
          units: idx, firstTry: firstTryCount, wrong: wrong + tipsUsed
        });
      }
      function updateHud() {
        hudQ.innerHTML = '';
      ...
      function finale() {
        ...
        later(function () {
          var score = Math.max(0, scoreNow());
          ui.showModal(d.finale.title,
            '<p>' + ui.escapeHtml(d.finale.text) + '</p>' +
            '<p class="lb-modal-note">Punkte: <b>' + score + '</b> · Fehlversuche: ' + wrong +
            ' · Tipps: ' + tipsUsed + '</p>');
          // Tipps rechnet dieses Level selbst ab (frageweise) -> als wrong-Äquivalent
          ctx.complete({ units: TOTAL, firstTry: firstTryCount, wrong: wrong + tipsUsed });
        }, fast ? 60 : 2400);
      }
    },
