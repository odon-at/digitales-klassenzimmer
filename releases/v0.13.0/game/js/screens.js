...
(function (NX) {
  ...
  function showScreen(id) {
    ...
    // die Einleitungs-Vertonung endet mit dem Bildschirm
    if (NX.audio && NX.audio.clip) NX.audio.clip.stop();
  ...
  function openInfo(lv) {
    ...
    if (NX.infoSystem && NX.infoSystem.has(lv.id)) {
      ...
      ui.showModal(lv.info.title, lv.info.html, { speak: lv.info.html });
    }
  }
  ...
  function renderAvatar() {
    ...
    D.avatars.forEach(function (a) {
      ...
      /* Kein „AUSWÄHLEN"-Knopf mehr (story/avatar.md): Ein Klick auf die Karte
         genügt. Damit sie nicht nur mit der Maus bedienbar ist, wird die Karte
         selbst zum Bedienelement – der Knopf war bisher der einzige
         Tastaturweg zur Auswahl. */
      var card = el('div', {
        class: 'avatar-card', dataset: { id: a.id },
        role: 'button', tabindex: '0', 'aria-pressed': 'false',
        'aria-label': a.name + ', ' + a.role + ', Lerntyp ' + a.learnType + ' – auswählen'
      }, [
      ...
      // beim Durchtabben soll die Vorschau genauso mitlaufen wie bei der Maus
      card.addEventListener('focus', function () { previewAvatar(a); });
  ...
  function highlightSelected() {
    ui.$$('.avatar-card').forEach(function (c) {
      var on = c.dataset.id === selectedAvatarId;
  ...
  /* Die Einleitung wird vorgelesen – unabhängig vom gewählten Avatar
     (story/einleitung.md). Anders als der VORLESEN-Knopf ist das eine feste
     Aufnahme, keine Sprachsynthese: Sie vertont genau diesen Text. */
  function introVoiceSync(playing) {
    var b = $('#btn-intro-voice');
    if (!b) return;
    b.textContent = playing ? '⏹ Vorlesen stoppen' : '🔊 Nochmal vorlesen';
  }
  function playIntroVoice() {
    var A = NX.audio;
    if (!A || !A.clip) return false;
    return A.clip.play('intro', {
      onStart: function () { introVoiceSync(true); },
      onDone: function () { introVoiceSync(false); },
      onUnavailable: function () { introVoiceSync(false); }
    });
  }
  ...
  function startIntro() {
    ...
    /* Bis hierher wurde mehrfach geklickt (Start → Login → Avatar → Bestätigen),
       die Tonfreigabe des Browsers liegt also vor. Fehlt die Datei oder ist der
       Ton stumm, bleibt der Knopf aus und nichts bricht. */
    var vBtn = $('#btn-intro-voice');
    if (vBtn) {
      var can = NX.audio && NX.audio.clip && NX.audio.clip.available('intro');
      vBtn.hidden = !can;
      introVoiceSync(false);
      if (can) playIntroVoice();
    }
  ...
  function openLevel(id) {
    ...
    renderTasks(lv);
    renderBonusHint(lv);
    // Aufgabenkarte beim Betreten IMMER sichtbar – ein Level darf sie für eine
    // Phase ausblenden (ctx.showTasks), das darf aber nicht hängen bleiben.
    showTasks(true);
    ...
    var ctx = {
      ...
      /* Blendet die gemeinsame Aufgabenkarte aus – für Phasen, in denen es
         nichts mehr abzuhaken gibt (Level 1, Schritt 3). Beim Betreten eines
         Levels setzt openLevel() sie wieder sichtbar. */
      showTasks: showTasks,
  ...
  /* ------------------------------------------------------------------ „Über uns“ */
  /* Eigenes Overlay statt ui.showModal: Das Fenster braucht ✕, Logos, einen
     externen Link und einen eigenen Aktionsknopf – das kann showModal nicht. */
  function openAbout() {
    var ov = $('#aboutscreen');
    if (!ov) return;
    ov.hidden = false;
    document.addEventListener('keydown', aboutEsc);
    try { $('#about-close').focus({ preventScroll: true }); } catch (e) { /* ignore */ }
  }
  function closeAbout() {
    var ov = $('#aboutscreen');
    ...
    try { $('#btn-about').focus({ preventScroll: true }); } catch (e) { /* ignore */ }
  }
  function aboutEsc(e) { if (e.key === 'Escape') { closeAbout(); } }

  /* Der Weg von „SPIEL STARTEN“ – auch „Jetzt spielen!“ im Über-uns-Fenster
     nimmt genau diesen, damit es nirgends zwei Startwege gibt. */
  ...
  function init() {
    ...
    // Über uns
    $('#btn-about').addEventListener('click', function () { NX.audio.play('click'); openAbout(); });
    $('#about-close').addEventListener('click', function () { NX.audio.play('click'); closeAbout(); });
    $('#aboutscreen').addEventListener('click', function (e) {
      if (e.target === $('#aboutscreen')) closeAbout();   // nur der abgedunkelte Rand
    });
    $('#about-play').addEventListener('click', function () { closeAbout(); startNewGame(); });
    ...
    $('#btn-intro-voice').addEventListener('click', function () {
      if (NX.audio.clip.isPlaying()) { NX.audio.clip.stop(); introVoiceSync(false); }
      else playIntroVoice();
    });
    ...
    $('#btn-level-tip').addEventListener('click', function () {
      ...
      NX.audio.play('click');
      // Vorgelesen wird nur auf Knopfdruck, immer per Browser-Stimme.
      ui.showModal('💡 TIPP ' + (tipsUsed + 1) + '/' + tips.length + ' · −' + TIP_COST + ' PUNKTE',
        tips[tipsUsed], { speak: tips[tipsUsed] });
      tipsUsed++;
    });
