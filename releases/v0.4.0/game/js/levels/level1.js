/* =========================================================================
   ...
   Konzept: Die internen Systemdaten der Stadt wurden vom Hacker manipuliert.
   Nur die unabhängigen Open-Data-Archive sind echt. Die Spieler:innen öffnen das
   Open-Data-Portal der Datenbehörde Nexus (URL eingeben → senden), lesen dort die
   echten Werte und korrigieren damit die gefälschten Einträge im internen System.
   Lernpunkt: Fehlerhafte Daten werden NICHT geraten, sondern aus einer
...
(function (NX) {
  ...
  NX.levelDefs.push({
    ...
    quality: null, // eigener Datenqualitäts-Balken im Body (siehe Spec-Layout)
    maxScore: 100,
    info: {
    ...
    mount: function (container, ctx) {
      ...
      /* ---- Open-Data-Portal (erst nach SENDEN sichtbar) ---- */
      ...
      /* ---- Internes (gehacktes) Stadt-System ---- */
      ...
      function tryFix(rowDef, cat, cell, input) {
        ...
        if (fixedCount >= badTotal) {
          solved = true;
          ctx.markTask(3);
          ctx.complete(Math.max(40, 100 - wrong * 5 - hints * 10));
        } else {
