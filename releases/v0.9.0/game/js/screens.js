...
(function (NX) {
  ...
  var kiGreetedTip = false;    // die KI-Aufnahme startet nur beim ersten Tipp von selbst
  ...
  function init() {
    ...
    $('#btn-level-tip').addEventListener('click', function () {
      ...
      NX.audio.play('click');
      /* Der Tipp wird vorgelesen; beim allerersten Tipp der Sitzung meldet
         sich zusätzlich die Bürger-KI mit ihrer eigenen Stimme (stimme.md). */
      ui.showModal('💡 TIPP ' + (tipsUsed + 1) + '/' + tips.length + ' · −' + TIP_COST + ' PUNKTE',
        tips[tipsUsed], { speak: tips[tipsUsed], autoKi: !kiGreetedTip });
      kiGreetedTip = true;
      tipsUsed++;
    });
