...
(function (NX) {
  ...
  /* Was der Vorlese-Knopf gerade sprechen würde. Jede Variante meldet hier
     den Text, der unter der Leiste sichtbar ist. */
  var spokenText = '';
  var kiGreeted = false;   // die KI-Aufnahme startet nur beim ersten Info-Öffnen je Sitzung
  ...
  function open(levelId, options) {
    ...
    /* Sprachleiste: liest den gerade sichtbaren Text vor und spielt auf Wunsch
       die Aufnahme der Bürger-KI (story/audio/stimme.md). Bei Lennox nicht –
       der Funk-Kanal hat seine eigene Transport-Leiste. */
    voiceEl.innerHTML = '';
    spokenText = inf.title + '. ' + (variant.headline || '');
    if (!inf[av.id] || av.id !== 'lennox') {
      voiceEl.appendChild(ui.voiceBar(function () { return spokenText; },
        { autoKi: !kiGreeted }));
      kiGreeted = true;
    }
  ...
  function renderZen(v, done) {
    ...
    function draw() {
      ...
      spokenText = ui.plainText(pane.innerHTML);   // Vorlese-Knopf liest die aktuelle Seite
