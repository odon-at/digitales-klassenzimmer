    ...
    mount: function (container, ctx) {
      ...
      function showPlausibilityBox(cell) {
        ...
        markBtn.addEventListener('click', function () {
          ctx.audio.play('click');
          fixbox.innerHTML = '';
          /* Die Entscheidungsfrage groß und unübersehbar (Spec „muss besonders
             dick, auffällig und gut sichtbar dargestellt werden"). */
          var box = el('div', {}, [
            el('p', { class: 'l1-reason', html:
              '<b>' + ui.escapeHtml(pl.field) + ':</b> ' + ui.escapeHtml(pl.reason) }),
            el('div', { class: 'instr-question', role: 'status' }, [
              el('span', { class: 'instr-question-ico', 'aria-hidden': 'true', text: '⚠' }),
              el('span', { text: pl.question })
            ])
          ]);
          /* Beide Optionen sind BEWUSST identisch gestaltet. Vorher hing die
             Button-Klasse am Richtig-Flag – die Lösung leuchtete also, bevor
             man nachdachte. Das Flag wertet jetzt nur noch choose() aus. */
          var acts = el('div', { class: 'choice-grid' });
          pl.options.forEach(function (o) {
            var b = el('button', { class: 'choice-btn', type: 'button' }, [
              el('span', { class: 'choice-btn-badge', 'aria-hidden': 'true', text: o.id.toUpperCase() }),
              el('span', { text: o.label })
            ]);
            b.addEventListener('click', function () { choose(o, cell, b); });
            acts.appendChild(b);
          });
      ...
      function choose(o, cell, btn) {
        if (!o.ok) {
          wrong++;
          ctx.audio.play('error');
          // sichtbar machen, was schon probiert wurde – die Frage bleibt offen
          if (btn) btn.classList.add('is-tried');
        ...
        fixbox.innerHTML = '';
        fixbox.appendChild(el('p', { class: 'l1-reason', text: '✓ ' + o.feedback }));
        countFix();
      }
