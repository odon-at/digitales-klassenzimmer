...
(function (NX) {
  ...
  var speech = { text: '', lines: null, clipKey: null, onLine: null, onDone: null, onUnavailable: null };
  function resetSpeech() {
    speech.text = ''; speech.lines = null; speech.clipKey = null;
  ...
  function open(levelId, options) {
    ...
    /* EINE Sprachleiste für alle drei Avatare (story/stimme.md). Sie startet
       nie von selbst und liest immer genau das vor, was gerade zu sehen ist. */
    voiceEl.innerHTML = '';
    resetSpeech();
    speech.text = inf.title + '. ' + (variant.headline || '');
    speech.clipKey = 'info-' + levelId + '-' + (inf[av.id] ? av.id : 'zen');
    voiceEl.appendChild(ui.voiceBar(
      function () { return speech.text; },
      {
        clipKey: function () { return speech.clipKey; },
