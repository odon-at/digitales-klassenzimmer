/* =========================================================================
   ...
   Drei Tonquellen, ein gemeinsamer Mute-Schalter:
     ...
     music         – leise Hintergrundschleife mit eigenem Knopf und eigener
                     Einstellung; 🔊 bleibt der Hauptschalter darüber
...
(function (NX) {
  ...
  function setMuted(v) {
    ...
    /* Stummschalten heißt: JETZT still – Sprachausgabe, Clips UND Musik.
       Im Klassenzimmer muss ein Griff genügen. Beim Aufheben kommt die Musik
       zurück, sofern sie eingeschaltet ist. */
    if (muted) {
      if (voice) voice.stop();
      if (clip) clip.stop();
      if (music) music.sync();
    } else if (music) { music.sync(); }
    return muted;
  }
  function toggle() { return setMuted(!muted); }

  var clip;    // weiter unten befüllt; voice.speak() stoppt darüber laufende Clips
  var music;   // dito; senkt sich, solange gesprochen wird
  ...
  /* ---- Hintergrundmusik -------------------------------------------------
     Leiser Loop aus game/media/, Pfad in js/data/voice.js. Eigener Knopf,
     eigene Einstellung (nexusdata.music) – 🔊 bleibt der Hauptschalter
     darüber. Fehlt die Datei, meldet available() false, der Knopf bleibt
     ausgeblendet und alles andere läuft unverändert weiter.
     Kein WebAudio-Buffer: der bräuchte fetch(), und das scheitert unter
     file://. Dafür kann der <audio>-Loop je nach Browser eine winzige Lücke
     haben – vertretbar für eine leise Untermalung.                          */
  music = (function () {
    var MUSIC_KEY = 'nexusdata.music';
    var VOL = 0.22;        // Effekte liegen bei 0.08–0.18 und bleiben klar darüber
    var VOL_DUCK = 0.07;   // während gesprochen wird
    var on = true;
    try { on = localStorage.getItem(MUSIC_KEY) !== '0'; } catch (e) {}

    var elm = null, ready = false, broken = false, ducked = false, armed = false;
    var onReadyCb = null;

    function track() { return NX.musicTrack || null; }

    function ensure() {
      if (elm || broken || !track()) return elm;
      if (typeof window.Audio !== 'function') { broken = true; return null; }
      try { elm = new window.Audio(track()); } catch (e) { broken = true; return null; }
      elm.loop = true;
      elm.volume = VOL;
      try { elm.preload = 'metadata'; } catch (e) { /* ignore */ }
      elm.addEventListener('error', function () {
        broken = true; ready = false;
        if (onReadyCb) onReadyCb(false);
      });
      elm.addEventListener('loadedmetadata', function () {
        ready = true;
        if (onReadyCb) onReadyCb(true);
        sync();
      });
      return elm;
    }

    function available() { return !!(track() && !broken && ready); }
    function isOn() { return on; }

    /* Spielt oder pausiert je nach Einstellung UND Hauptschalter. */
    function sync() {
      var a = ensure();
      if (!a) return false;
      if (!on || muted || !armed) {
      ...
      var p;
      try { p = a.play(); } catch (e) { broken = true; return false; }
      if (p && typeof p['catch'] === 'function') {
        // Autoplay-Sperre: kein Fehler, wir versuchen es beim nächsten Klick erneut
    ...
    function toggleMusic() { return setOn(!on); }

    function duck(v) {
      ducked = !!v;
      if (elm) { try { elm.volume = ducked ? VOL_DUCK : VOL; } catch (e) { /* ignore */ } }
    }

    /* Browser lassen Ton erst nach einer Nutzeraktion zu. arm() wartet auf den
       ersten Klick und startet die Musik dann – niemals vorher. */
    function arm() {
      if (armed) return;
      var go = function () {
        armed = true;
      ...
      try { document.addEventListener('click', go, true); } catch (e) { /* ignore */ }
      ensure();
    }

    function onReady(fn) { onReadyCb = fn; if (ready) fn(true); else if (broken) fn(false); }

    try { window.addEventListener('beforeunload', function () {
      if (elm) { try { elm.pause(); } catch (e) { /* ignore */ } }
    }); } catch (e) { /* ignore */ }

    return {
      arm: arm, sync: sync, setOn: setOn, toggle: toggleMusic, isOn: isOn,
      available: available, duck: duck, onReady: onReady,
      isPlaying: function () { return !!(elm && !elm.paused); },
      volume: function () { return elm ? elm.volume : VOL; }
    };
  })();

  NX.audio = {
    play: play, isMuted: isMuted, setMuted: setMuted, toggle: toggle,
    voice: voice, clip: clip, music: music
  };
})(window.NX);
