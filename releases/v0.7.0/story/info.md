...
**Der Tipp-Button (Die Notfall-Hilfe):**
  * **Kosten:** **Minuspunkte** – umgesetzt sind einheitlich **−10 Punkte** je Tipp.
...
(`lyra` / `lennox` / `zen`), je einer Bonusfrage und `bonusPoints` (15).
...
**Bonuspunkte:** `state.bonuses` (`{ levelId: Punkte }`). Auch eine **falsche** Antwort
wird mit 0 gespeichert – damit ist die Frage verbraucht und lässt sich nicht durchprobieren.

**Tipp-Kosten:** zentral in `game/js/screens.js` (`TIP_COST = 10`), Abzug einmalig in
