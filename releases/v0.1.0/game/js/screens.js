...
(function (NX) {
  ...
  function renderStart() {
    ...
    cont.hidden = !has;
    // Build-Version anzeigen (Quelle: NX.build aus js/version.js)
    var credits = $('#screen-start .credits');
    var b = window.NX.build || {};
    if (credits && b.version) {
      credits.textContent = 'Open Data Classroom · von Sarah & Chiara Hamedinger · v' + b.version;
    }
  }
  ...
  function renderMap() {
    ...
    L.all().forEach(function (lv, idx) {
      ...
      var statusTxt = done ? '✔ Fragment gesichert' : (open ? '▶ bereit' : '🔒 gesperrt');
